...
from tlt.data import create_token_label_loader, create_token_label_dataset
from util.losses import TokenLabelGTCrossEntropy, TokenLabelCrossEntropy, TokenLabelSoftTargetCrossEntropy
...
def get_args_parser():
    ...
    # Model parameters
    parser.add_argument('--model', default='svt_b', type=str, metavar='MODEL',
...
def build_token_label(args):
    ...
    dataset_train = create_token_label_dataset(
        '', root=args.data_root, label_root=args.token_label_data)
...
def main(args):
    ...
    if args.token_label:
        criterion = TokenLabelCrossEntropy(dense_weight=args.dense_weight, cls_weight=args.cls_weight, mixup_active=False).cuda()
    else:
